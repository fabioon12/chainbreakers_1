...
@export var gravity: float = 7.8
@export var jump_force: int = 4
@export var walk_sped: int = 1
@export var run_speed: int = 16
...
func _physics_process(delta: float) -> void:
	...
	if !is_dying:
		...
		if (Input.is_action_pressed("forward") || Input.is_action_pressed("backward") || Input.is_action_pressed("left")  || Input.is_action_pressed("right")):
			...
			if Input.is_action_just_released("spirnt") and (is_walking):
				movement_speed = run_speed
				is_running = true
			else :
				is_walking = true
				movement_speed = walk_sped
		else:
			is_walking = false
			is_running = false
